...
export default function RelatoriosPage() {
  ...
  async function fetchAll() {
    ...
    // 3) Funil por período (mês/origem/campanha/status)
  ...
  // Normaliza daily para gráficos mesmo se os nomes das colunas mudarem
  const dailyChart = useMemo(() => {
    return (daily ?? []).map((r) => {
      ...
      return {
        day: dayIso ? formatDateShort(dayIso) : "",
        leads,
        sales,
        revenue,
      };
  ...
  // Cards: tenta pegar de v_funnel_dashboard, se não existir, calcula um básico pelo v_funnel_by_period (mês atual)
  const kpis = useMemo(() => {
    // Preferência: vem pronto do dashboard
    if (dash) {
    ...
    // Fallback: básico pelo funil por período no mês mais recente presente
  ...
  // Agrega "status atual" para gráfico de funil (a partir da tabela por período filtrada)
  const funnelAgg = useMemo(() => {
    ...
    // ordena por qty desc só pra ficar visual
      ...
      <div style={shell}>
        <div style={{ display: "flex", justifyContent: "space-between", gap: 10, flexWrap: "wrap" }}>
          ...
          <div style={{ display: "flex", gap: 10, flexWrap: "wrap", alignItems: "center" }}>
            {loading ? <span style={chipStyle("primary")}>Carregando…</span> : <span style={chipStyle("muted")}>OK</span>}
            <button
              onClick={fetchAll}
              style={{
                background: "rgba(255,255,255,0.06)",
