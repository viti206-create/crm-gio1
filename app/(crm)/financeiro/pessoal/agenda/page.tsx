...
export default function AgendaFinanceiraPessoalPage() {
  ...
  function clearEditQuery() {
    ...
    router.replace(next ? `/financeiro/pessoal/agenda?${next}` : "/financeiro/pessoal/agenda");
  }
  ...
  async function fetchData() {
    ...
    const nextRows = (data as Schedule[]) || [];
    setRows(nextRows);
    setLoading(false);
  ...
  async function generateTransaction(schedule: Schedule) {
    ...
    const currentGeneratedCount = Number(schedule.generated_count ?? 0);
    ...
    <div style={pageStyle}>
      ...
      >
        <div style={{ display: "grid", gap: 8 }}>
          <div style={{ fontSize: 22, fontWeight: 900, letterSpacing: 0.2 }}>
            Agenda Financeira Pessoal
          </div>
          <div style={chipStyle}>Scope: personal</div>
        </div>

        <div style={{ display: "flex", gap: 10, flexWrap: "wrap" }}>
          <Link href="/financeiro/pessoal" style={btnStyle}>
            Voltar
          </Link>
      ...
      <div style={cardStyle}>
        ...
        >
          <div>
            ...
            <input
              style={inputStyle}
              placeholder="Ex.: Aluguel / Escola / Internet / Salário"
              value={description}
          ...
          <button type="button" onClick={saveSchedule} style={btnPrimaryStyle} disabled={saving}>
      ...
      <div style={cardStyle}>
        ...
        {loading ? (
          ...
          <div style={{ opacity: 0.75 }}>Nenhum compromisso cadastrado ainda.</div>
        ) : (
              ...
              <tbody>
                {rows.map((r) => (
                  <tr key={r.id}>
                    ...
                    <td style={{ padding: "10px 0", borderTop: "1px solid rgba(255,255,255,0.06)", fontWeight: 800 }}>
                    ...
                    <td style={{ padding: "10px 0", borderTop: "1px solid rgba(255,255,255,0.06)" }}>
                      {r.last_generated_date ? formatDateBR(r.last_generated_date) : "—"}
                    </td>
                    <td style={{ padding: "10px 0", borderTop: "1px solid rgba(255,255,255,0.06)" }}>
                      <div style={{ display: "flex", gap: 8, flexWrap: "wrap" }}>
                        <button type="button" onClick={() => handleEdit(r)} style={btnStyle}>
                          Editar
                        </button>
                        <button type="button" onClick={() => generateTransaction(r)} style={btnPrimaryStyle}>
                          Gerar lançamento
                        </button>
                        <button type="button" onClick={() => remove(r.id)} style={btnDangerStyle}>
                          Excluir
                        </button>
                      </div>
                    </td>
