    ...
    <div ref={setNodeRef} style={style}>
      <div style={headerStyle}>
        <div style={titleStyle}>
          ...
          {stage.is_final ? (
            <span style={{ ...chipStyle("muted"), opacity: 0.9 }}>Final</span>
          ) : null}
        </div>
    ...
    >
      <div
        ...
        style={{
          ...
          background:
            "linear-gradient(180deg, rgba(18,18,24,0.96) 0%, rgba(10,10,14,0.94) 100%)",
...
function DashboardPageInner() {
  ...
  function setLeadParam(leadId: string | null) {
  ...
  // ✅ abre modal automaticamente se vier /dashboard?lead=ID
  useEffect(() => {
    const leadFromUrl = searchParams.get("lead");
    if (!leadFromUrl) return;

    // quando leads ainda não carregaram, aguarda
  ...
  async function onDragEnd(event: DragEndEvent) {
    ...
    if (targetStageId !== activeLeadNow.stage_id) {
      ...
      if (error) {
        ...
        alert("Não consegui mover o lead. Veja o console (F12).");
        return;
      }

      if (selectedLeadId === activeLeadNow.id) {
        loadActivities(activeLeadNow.id);
      }
  ...
  async function handleAddNote() {
    ...
    if (error) {
      ...
      alert("Não consegui salvar a observação (policy/RLS). Veja o console.");
      return;
    }
  ...
  async function handleSetNextAction() {
    ...
    if (!nextActionAt) {
      alert("Escolha a data/hora da próxima ação.");
      return;
    }
    ...
    if (error) {
      ...
      alert("Não consegui salvar a próxima ação (policy/RLS). Veja o console.");
      return;
    }
  ...
  return (
    <div>
      ...
      >
        <Link href="/leads/new" style={actionBtnPrimary}>
          + Novo lead
        </Link>
        <button onClick={fetchData} style={actionBtn}>
          Atualizar
        </button>
      </div>
          ...
          {activeLead ? (
            ...
            >
              ...
              <div style={{ opacity: 0.9 }}>
                {activeLead.phone_raw ?? activeLead.phone_e164}
              </div>
            ...
            <div style={{ display: "grid", gap: 10 }}>
              <div style={{ display: "flex", gap: 10, flexWrap: "wrap" }}>
                <span style={chipStyle("primary")}>
                  Etapa: {stageNameFromId(selectedLead.stage_id)}
                </span>
                <span style={chipStyle("muted")}>
                  Telefone:{" "}
                  {normalizePhoneReadable(selectedLead.phone_raw, selectedLead.phone_e164)}
                </span>
              ...
              <div style={softText}>
                Dica: “Abrir WhatsApp com mensagem” já leva o texto pronto; se preferir, use
                “Copiar mensagem pronta”.
              </div>
            </div>
